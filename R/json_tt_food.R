...
json_tt_food_deid <- function() {
  
  tt_food_deid_list <- list(
    participant_id = list( Description = 'participant id number'),
    session_id = list( Description = 'BIDS session ID indicating when data was collected',
                       Levels = list ('ses-baseline' = 'baseline',
                                      'ses-followup' = '1-year follow-up')),
  ...
  # convert formatting to JSON
  tt_food_deid_json <- RJSONIO::toJSON(tt_food_deid_list, pretty = TRUE)
  
  # double check
  if (isFALSE(RJSONIO::isValidJSON(tt_food_deid_json, asText = TRUE))){
    print('Double-entered Taste-Test samples JSON file may be invalid')
  }
  
  return(tt_food_deid_json)
