...
#'
#' This function generates a json file for the scored Child Feeding Questionnaire and raw participant responses. This function provides accurate json files ONLY if data is processed using score_cfq function in dataprepr and is only accurate for data collected in Study BRAKE.
#'
...
json_cfq_deid <- function() {
  ...
  # convert formatting to JSON
  cfq_deid_json <- RJSONIO::toJSON(cfq_deid_list, pretty = TRUE)
  
  # double check
  if (isFALSE(RJSONIO::isValidJSON(cfq_deid_json, asText = TRUE))){
    print('CFQ JSON file may be invalid')
  }
  
  return(cfq_deid_json)
