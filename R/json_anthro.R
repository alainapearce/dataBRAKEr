...
json_anthro_deid <- function() {
  
  anthro_deid_list <- list(
    participant_id = list( Description = 'participant id number'),
    session_id = list( Description = 'BIDS session ID indicating when data was collected',
                       Levels = list ('ses-baseline' = 'baseline',
                                      'ses-followup' = '1-year follow-up')),
  ...
  # convert formatting to JSON
  anthro_deid_json <- RJSONIO::toJSON(anthro_deid_list, pretty = TRUE)
  
  # double check
  if (isFALSE(RJSONIO::isValidJSON(anthro_deid_json, asText = TRUE))){
    print('anthro info JSON file may be invalid')
  }
  
  return(anthro_deid_json)
