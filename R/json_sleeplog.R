...
json_sleeplog_deid <- function() {
  
  sleeplog_deid_list <- list(
    'MeasurementToolMetadata' = list(
      Description = 'Home week-long sleep log'),
    participant_id = list( Description = 'participant id number'),
    session_id = list( Description = 'BIDS session ID indicating when data was collected',
  ...
  # convert formatting to JSON
  sleeplog_deid_json <- RJSONIO::toJSON(sleeplog_deid_list, pretty = TRUE)
  
  # double check
  if (isFALSE(RJSONIO::isValidJSON(sleeplog_deid_json, asText = TRUE))){
    print('Sleep Log JSON file may be invalid')
  }
  
  return(sleeplog_deid_json)
