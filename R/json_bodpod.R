...
json_bodpod_deid <- function() {
  
  bodpod_deid_list <- list(
    participant_id = list( Description = 'participant id number'),
    session_id = list( Description = 'BIDS session ID indicating when data was collected',
                       Levels = list ('ses-baseline' = 'baseline',
                                      'ses-followup' = '1-year follow-up')),
  ...
  # convert formatting to JSON
  bodpod_deid_json <- RJSONIO::toJSON(bodpod_deid_list, pretty = TRUE)
  
  # double check
  if (isFALSE(RJSONIO::isValidJSON(bodpod_deid_json, asText = TRUE))){
    print('BodPod JSON file may be invalid')
  }
  
  return(bodpod_deid_json)
