...
json_demo_deid <- function() {
  ...
  demo_deid_list <- list(
    ...
    sex = list( Description = 'Child sex',
                Levels = list ('male' = 'male',
                               'female' = 'female')),
    age = list( Description = 'Child age rounded down to the nearest half year for deidentification purposes. Contact azp271@psu.edu if want access to full data.',
                Units = 'years'),
    ethnicity = list( Description = 'Parent-reported child ethnicity. Collected at ses-baseline only but values applied to ses-baseline and ses-followup',
                      Levels = list ('0' = 'Not Hispanic or Latino',
                                     '1' = 'Hispanic or Latino')),
    race = list( Description = 'Parent-reported child race. Collected at ses-baseline only but values applied to ses-baseline and ses-followup. Note: for deidentification purposes, reduced to categories white and non-white.',
                 Levels = list ('0' = 'Race other than white',
                                '1' = 'White')),
    child_other_race = list( Description = 'If other race, please specify'),
    demo_income = list( Description = 'Including ALL sources (such as social security income, child support payments, government assistance, dividends from investments, etc.) what was your household\'s combined yearly income last year BEFORE taxes? For deidentificaiton purposes, reduced to 3 categories.',
                        Levels = list ('0' = 'Less than or equal to $50,000',
                                       '1' = '$51,000 - $100,000',
                                       '2' = '$100,000 +')),
    demo_mom_ed = list( Description = 'What is the child\'s biological mother\'s highest level of formal education?. For deidentificaiton purposes, reduced to 3 categories.',
                        Levels = list ('0' = 'Associates Degree or less',
                                       '1' = '4 year degree',
                                       '2' = '> 4 year degree')),
    demo_ded_ed = list( Description = 'What is the child\'s biological father\'s highest level of formal education? For deidentificaiton purposes, reduced to 3 categories.',
                        Levels = list ('0' = 'Associates Degree or less',
                                       '1' = '4 year degree',
                                       '2' = '> 4 year degree')),
    child_bmi = list( Description = 'child body mass index computed from height_mean_cm and weight_mean_kg',
  ...
  # convert formatting to JSON
  demo_deid_json <- RJSONIO::toJSON(demo_deid_list, pretty = TRUE)
  
  # double check
  if (isFALSE(RJSONIO::isValidJSON(demo_deid_json, asText = TRUE))){
    print('demographic info JSON file may be invalid')
  }
  
  return(demo_deid_json)
