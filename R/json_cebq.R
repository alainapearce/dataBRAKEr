...
#'
#' This function generates a json file for the scored Children's Eating Behavior Questionnaire and raw participant responses. This function provides accurate json files ONLY if data is processed using score_cebq function in dataprepr and is only accurate for data collected in Study BRAKE.
#'
...
json_cebq_deid <- function() {
  ...
  # convert formatting to JSON
  cebq_deid_json <- RJSONIO::toJSON(cebq_deid_list, pretty = TRUE)
  
  # double check
  if (isFALSE(RJSONIO::isValidJSON(cebq_deid_json, asText = TRUE))){
    print('CEBQ JSON file may be invalid')
  }
  
  return(cebq_deid_json)
