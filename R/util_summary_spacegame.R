#' util_summary_spacegame: Process cleaned data from the Space Game (2-stage reinforcement learning task) to generate summary metrics
#'
#' This function generates summary metrics that can be compiled (e.g., sapply function) to create a behavioral database
#'
...
#' spacegame_task_pardat <- util_summary_spacegame(sub_str, ses, data_path, return = TRUE)
#'
#' \dontrun{
#' }
#'
...
util_summary_spacegame <- function(sub_str, ses, base_wd, overwrite = FALSE, return_data = FALSE) {
  ...
  data_file <- paste0(raw_wd, slash, sub_str, '_ses-', ses, '_task-spacegame_beh.tsv')
  ...
  # re-order columns
  dat <- dat[c('sub', 'date', 'block', 'trial', 'timeout_earth', 'timeout_planet', 'state_earth', 'state_planet', 'stim_left', 'stim_right', 'rt_earth', 'rt_planet', 'choice_earth', 'response', 'points', 'stake', 'score', 'rewards1', 'rewards2', 'missed_earth', 'missed_planet', 'stay_planet')]
  ...
  if (!file.exists(paste0(raw_wd, sub_str, '_task-spacegame_beh.tsv')) | isTRUE(overwrite)) {
    write.table(dat, paste0(raw_wd, sub_str, '_ses-', ses, '_task-spacegame_beh.tsv'), sep='\t', quote = FALSE, row.names = FALSE, na = 'NaN')
