#' json_spacegame_dm: Generates a json file for the Space Game decision making model summary output from matlab
#'
#' This function generates a json file for summary decision making model results save in the derivatives/beh file
#'
#' @return A json file documenting the summary decision making model results derivatives/beh file
#'
#'
#'
#' @export

json_spacegame_dm <- function() {
  
  spacegame_dm_list <- list(
  ...
  spacegame_dm_json <- RJSONIO::toJSON(spacegame_dm_list, pretty = TRUE)
  
  # double check
  if (isFALSE(RJSONIO::isValidJSON(spacegame_dm_json, asText = TRUE))){
    print('spacegame dm JSON file may be invalid')
  }
  
  return(spacegame_dm_json)
