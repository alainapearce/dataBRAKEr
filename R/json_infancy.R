...
json_infancy <- function() {
  ...
  infancy_list <- list(
    'MeasurementToolMetadata' = list(
      Description = 'Questions related to the child\'s birth and infancy that parents answered as part of the REDCap demographics form'),
...
json_infancy_deid <- function() {
  
  infancy_deid_list <- list(
    'MeasurementToolMetadata' = list(
      Description = 'Questions related to the child\'s birth and infancy that parents answered as part of the REDCap demographics form'),
    participant_id = list( Description = 'participant id number'),
    session_id = list( Description = 'BIDS session ID indicating when data was collected',
  ...
  # convert formatting to JSON
  infancy_deid_json <- RJSONIO::toJSON(infancy_deid_list, pretty = TRUE)
  
  # double check
  if (isFALSE(RJSONIO::isValidJSON(infancy_deid_json, asText = TRUE))){
    print('Infancy JSON file may be invalid')
  }
  
  return(infancy_deid_json)
