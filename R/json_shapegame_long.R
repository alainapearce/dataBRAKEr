#' json_shapegame_long: Generates a json file for the Shape Game long derivative file
#'
#' This function generates a json file for the long derivative Shape Game database
#'
#' @return A json file documenting long formatted Shape Game database
#'
#'
#' @export

json_shapegame_long <- function() {
  
  shapegame_long_list <- list(
  ...
  shapegame_long_json <- RJSONIO::toJSON(shapegame_long_list, pretty = TRUE)
  
  # double check
  if (isFALSE(RJSONIO::isValidJSON(shapegame_long_json, asText = TRUE))){
    print('shapegame_long JSON file may be invalid')
  }
  
  return(shapegame_long_json)
  
}

json_shapegame_long_deid <- function() {
  
  shapegame_long_deid_list <- list(
    'MeasurementToolMetadata' = list(
      Description = 'The Shape Game is a Value-Modulated Attentional Capture task modified for use with children.',
      Reference = 'Sali, A. W., Anderson, B. A., Yantis, S., Mostofsky, S. H., & Rosch, K. S. (2018). Reduced value-driven attentional capture among children with ADHD compared to typically developing controls. Journal of Abnormal Child Psychology, 46(6), 1187-1200. doi: 10.1007/s10802-017-0345-y.', 
      TermURL = 'https://pubmed.ncbi.nlm.nih.gov/28913698/',
  ...
  # convert formatting to JSON
  shapegame_long_deid_json <- RJSONIO::toJSON(shapegame_long_deid_list, pretty = TRUE)
  
  # double check
  if (isFALSE(RJSONIO::isValidJSON(shapegame_long_deid_json, asText = TRUE))){
    print('shapegame_long JSON file may be invalid')
  }
  
  return(shapegame_long_deid_json)
