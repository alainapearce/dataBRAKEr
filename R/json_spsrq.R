...
#'
#' This function generates a json file for the scored Sensitivity to Punishment and Sensitivity to Reward Questionnaire and raw participant responses. This function provides accurate json files ONLY if data is processed using score_spsrq function in dataprepr and is only accurate for data collected in Study BRAKE.
#'
...
json_spsrq_deid <- function() {
  
  spsrq_deid_list <- list(
    'MeasurementToolMetadata' = list(
      Description = 'Sensitivity to Punishment and Sensitivity to Reward Questionnaire.',
      Reference = 'Colder CR, Trucco EM, Lopez HI, et al. Revised reinforcement sensitivity theory and laboratory assessment of BIS and BAS in children. Journal of Research in Personality. 2011;45(2):198-207. doi:10.1016/j.jrp.2011.01.005', 
      TermURL = 'https://pubmed.ncbi.nlm.nih.gov/21603055/'),
  ...
  # convert formatting to JSON
  spsrq_deid_json <- RJSONIO::toJSON(spsrq_deid_list, pretty = TRUE)
  
  # double check
  if (isFALSE(RJSONIO::isValidJSON(spsrq_deid_json, asText = TRUE))){
    print('spsrq JSON file may be invalid')
  }
  
  return(spsrq_deid_json)
