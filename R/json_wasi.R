...
json_wasi_deid <- function() {
  ...
  # convert formatting to JSON
  wasi_deid_json <- RJSONIO::toJSON(wasi_deid_list, pretty = TRUE)
  
  # double check
  if (isFALSE(RJSONIO::isValidJSON(wasi_deid_json, asText = TRUE))){
    print('WASI JSON file may be invalid')
  }
  
  return(wasi_deid_json)
