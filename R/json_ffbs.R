...
#'
#' This function generates a json file for the scored Family Food Behavior Survey and raw participant responses. This function provides accurate json files ONLY if data is processed using score_ffbs function in dataprepr and is only accurate for data collected in Study BRAKE.
#'
...
json_ffbs_deid <- function() {
  ...
  # convert formatting to JSON
  ffbs_deid_json <- RJSONIO::toJSON(ffbs_deid_list, pretty = TRUE)
  
  # double check
  if (isFALSE(RJSONIO::isValidJSON(ffbs_deid_json, asText = TRUE))){
    print('ffbs JSON file may be invalid')
  }
  
  return(ffbs_deid_json)
